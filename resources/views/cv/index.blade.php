<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="description" content="Full-Stack PHP Developer">
    <meta name="keywords" content="resume,cv,Fabricio Zeferino">
    <meta name="author" content="Fabricio Zeferino">
    <meta name="robots" content="noindex">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link
        href="https://fonts.googleapis.com/css?family=Fira+Sans:200,200i,300,300i,400,400i,500,500i,600,600i,700,700i,800,800i,900,900i&display=swap"
        rel="stylesheet">
    <link href="{{ mix('css/app.css') }}" rel="stylesheet">
    <title>Fabricio Zeferino — Full-Stack PHP Developer</title>
</head>
<body>
<div id="cv" class="font-sans leading-relaxed text-gray-900 antialiased hyphens-manual optimize-legibility">

    <!-- Page -->
    <div class="page mx-auto max-w-a4 md:h-a4 p-6 sm:p-8 md:p-16 bg-white">

        <div class="md:col-2 print:col-2 col-gap-md md:h-a4-col print:h-a4-col col-fill">
            <header class="flex items-center mb-8">
                <div
                    class="initials-container mr-5 text-base leading-none text-white bg-gray-700 font-medium print:bg-black px-3 py-2">
                    <div class="initial text-center">F</div>
                    <div class="initial text-center">Z</div>
                </div>
                <h1 class="text-2xl font-bold uppercase text-gray-900 tex">Fabricio Zeferino</h1>
            </header>
            <section>

                <div class="col-break-avoid">

                    <h2 class="mb-4 text-sm text-gray-600 font-bold print:font-normal tracking-widest">ABOUT ME</h2>

                    <section class="mb-2 col-break-avoid">
                        <header>
                            <h3 class="text-lg text-gray-700 font-semibold leading-snug">Full-Stack PHP Developer</h3>
                            <p class="text-gray-700 leading-normal">Since 2017</p>
                        </header>
                        <p class="mt-2  text-gray-700 leading-normal">I am a passionate self-taught developer. I’ve
                            delivered working application utilising Laravel, JavaScript, jQuery, Bootstrap, MySQL and
                            mongoDB. I’m currently teaching myself TDD, Vue.js, Vuex, SPA, RESTful API and Tailwind
                            CSS.</p>
                    </section>

                </div>

                <section class="mb-4 col-break-avoid">
                    <p class="mt-1  text-gray-700 leading-normal">I started to study Web development in 2017, in 2018
                        during all year I have taught my self full-time. At the end of 2018, I have reached my object
                        getting my first job on Web Development.</p>
                </section>

            </section>

            <section class="mt-8">

                <!-- To keep in the same column -->
                <div class="col-break-avoid">

                    <h2 class="mb-4 text-sm text-gray-600 font-bold print:font-normal tracking-widest">EXPERIENCE</h2>

                </div>

                <section class="mb-4 col-break-avoid">
                    <header>
                        <h3 class="text-lg text-gray-700 font-semibold leading-snug">Mammoth GmbH</h3>
                        <p class="text-gray-700 leading-normal">Feb 2017 – Apr 2018 | Android Developer</p>
                    </header>
                    <p class="mt-1 pl-7  text-gray-700 leading-normal">
                        <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1 flex-shrink-0 text-gray-600 print:text-gray-900"
                             xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                            <path class="fill-current"
                                  d="M12 22a10 10 0 1 1 0-20 10 10 0 0 1 0 20zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm-2.3-8.7l1.3 1.29 3.3-3.3a1 1 0 0 1 1.4 1.42l-4 4a1 1 0 0 1-1.4 0l-2-2a1 1 0 0 1 1.4-1.42z"/>
                        </svg>
                        Wrote doner pork chop t-bone ham meatloaf ve&shy;ni&shy;sons. Ribeye turducken pancetta sausage.
                        Biltong atl. Yamoi.
                    </p>
                    <p class="mt-1 pl-7  text-gray-700 leading-normal">
                        <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1 flex-shrink-0 text-gray-600 print:text-gray-900"
                             xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                            <path class="fill-current"
                                  d="M12 22a10 10 0 1 1 0-20 10 10 0 0 1 0 20zm0-2a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm-2.3-8.7l1.3 1.29 3.3-3.3a1 1 0 0 1 1.4 1.42l-4 4a1 1 0 0 1-1.4 0l-2-2a1 1 0 0 1 1.4-1.42z"/>
                        </svg>
                        Tested beef rump beef ribs, shankle corned beef neck. Ribeye turducken pancetta sausage. Biltong
                        atl. Yamoi.
                    </p>
                </section>

                <section class="mb-4 col-break-avoid">


                </section>

                <section class="mt-8">

                    <!-- To keep in the same column -->
                    <div class="col-break-avoid">

                        <h2 class="mb-4 text-sm text-gray-600 font-bold print:font-normal tracking-widest">
                            EDUCATION</h2>

                        <section class="mb-4 col-break-avoid">
                            <header>
                                <h3 class="text-lg text-gray-800 font-semibold leading-snug">Bachelor in Business
                                    Administration</h3>
                                <p class="text-gray-800 leading-normal">2006 – 2010 | Bachelor in Business
                                    Administration</p>
                                <p class="leading-normal">This degree gave me technical skills and knowledge such as
                                    project management, resource management, communication, team-working and
                                    problem-solving.</p>
                            </header>
                        </section>

                    </div>


                </section>

                <section class="mt-8 bg-gray-100 py-2 px-4">

                    <!-- To keep in the same column -->
                    <div class="col-break-avoid">

                        <h2 class="mb-4 text-2xl text-gray-800 font-extrabold print:font-normal tracking-widest">Key
                            skills</h2>

                        <section class="mb-4 col-break-avoid">
                            <header>
                                <h3 class="text-lg text-gray-700 font-semibold leading-snug">
                                    <a href="https://github.com/WebPraktikos" class="group">Universal Resume
                                        <svg
                                            class="inline -mt-1 w-5 text-gray-600 group-hover:text-gray-700 change-color-fast print:text-gray-900"
                                            xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                            <path class="fill-current"
                                                  d="M19 6.41L8.7 16.71a1 1 0 1 1-1.4-1.42L17.58 5H14a1 1 0 0 1 0-2h6a1 1 0 0 1 1 1v6a1 1 0 0 1-2 0V6.41zM17 14a1 1 0 0 1 2 0v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7c0-1.1.9-2 2-2h5a1 1 0 0 1 0 2H5v12h12v-5z"/>
                                        </svg>
                                    </a>
                                </h3>
                                <p class="text-gray-700 leading-normal">Since 2019 | HTML CSS</p>
                            </header>
                            <p class="mt-1  text-gray-700 leading-normal">Filet mignon burgdoggen tri-tip swine pork
                                belly
                                ham hock flank pork. Ribeye turducken pancetta sausage. Biltong atl. Bacon ham shankle,
                                landjaeg pastrami beef.</p>
                        </section>

                    </div>

                    <section class="mb-4 col-break-avoid">
                        <header>
                            <h3 class="text-lg text-gray-700 font-semibold leading-snug">
                                <a href="https://github.com/WebPraktikos" class="group">fade-on-scroll
                                    <svg
                                        class="inline -mt-1 w-5 text-gray-600 group-hover:text-gray-700 change-color-fast print:text-gray-900"
                                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                        <path class="fill-current"
                                              d="M19 6.41L8.7 16.71a1 1 0 1 1-1.4-1.42L17.58 5H14a1 1 0 0 1 0-2h6a1 1 0 0 1 1 1v6a1 1 0 0 1-2 0V6.41zM17 14a1 1 0 0 1 2 0v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7c0-1.1.9-2 2-2h5a1 1 0 0 1 0 2H5v12h12v-5z"/>
                                    </svg>
                                </a>
                            </h3>
                            <p class="text-gray-700 leading-normal">2017 | JavaScript</p>
                        </header>
                        <p class="mt-1  text-gray-700 leading-normal">Strip steak tail capicola alcatra ground round
                            tenderloin ar. Venison tri-tip porchetta, brisket tenderloin pig beef ribies. Bacon ham
                            shankle,
                            landjaeg pastrami pork ribs.</p>
                    </section>


                </section>

                <section class="mt-8 py-2 px-4">

                    <!-- To keep in the same column -->
                    <div class="col-break-avoid mb-4" v-for="skill in skills">

                        <h2 class="mb-4 text-sm text-gray-600 font-bold print:font-normal tracking-widest">SKILLS</h2>

                        <section class="mb-4 col-break-avoid">
                            <header>
                                <h3 class="text-lg text-gray-700 font-semibold leading-snug">@{{ skill.title }}</h3>
                                <p class="text-gray-700 leading-normal">@{{ skill.sub_title }}</p>
                            </header>
                            <p class="mt-1  text-gray-700 leading-normal">@{{ skill.description }}</p>
                            <ul class="mt-1 mb-6 flex flex-wrap leading-normal">

                                <li v-for="tag in skill.tags"
                                    class="px-3 mr-1 mt-1 text-base text-gray-700 leading-relaxed print:bg-white print:border-inset bg-gray-200">
                                    @{{ tag.tag }}
                                </li>
                            </ul>
                        </section>


                    </div>


                </section>

                <section class="mt-8 py-2 px-4">

                    <!-- To keep in the same column -->
                    <div class="col-break-avoid">

                        <h2 class="mb-4 text-sm text-gray-600 font-bold print:font-normal tracking-widest">CONTACT</h2>

                        <section class="mb-4 col-break-avoid">
                            <ul>
                                <li class="mt-1 pl-7  text-gray-700 leading-normal">
                                    <a href="//webpraktikos.com" class="group">
                                        <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1" xmlns="http://www.w3.org/2000/svg"
                                             viewBox="0 0 24 24">
                                            <path
                                                class="text-gray-600 group-hover:text-gray-700 change-color-fast print:text-gray-900 fill-current"
                                                d="M19.48 13.03A4 4 0 0 1 16 19h-4a4 4 0 1 1 0-8h1a1 1 0 0 0 0-2h-1a6 6 0 1 0 0 12h4a6 6 0 0 0 5.21-8.98L21.2 12a1 1 0 1 0-1.72 1.03zM4.52 10.97A4 4 0 0 1 8 5h4a4 4 0 1 1 0 8h-1a1 1 0 0 0 0 2h1a6 6 0 1 0 0-12H8a6 6 0 0 0-5.21 8.98l.01.02a1 1 0 1 0 1.72-1.03z"/>
                                        </svg>
                                        webpraktikos.com</a>
                                </li>
                                <li class="mt-1 pl-7  text-gray-700 leading-normal">
                                    <a href="//webpraktikos.com" class="group">
                                        <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1" xmlns="http://www.w3.org/2000/svg"
                                             viewBox="0 0 24 24">
                                            <path
                                                class="text-gray-600 group-hover:text-gray-700 change-color-fast print:text-gray-900 fill-current"
                                                d="M19.48 13.03A4 4 0 0 1 16 19h-4a4 4 0 1 1 0-8h1a1 1 0 0 0 0-2h-1a6 6 0 1 0 0 12h4a6 6 0 0 0 5.21-8.98L21.2 12a1 1 0 1 0-1.72 1.03zM4.52 10.97A4 4 0 0 1 8 5h4a4 4 0 1 1 0 8h-1a1 1 0 0 0 0 2h1a6 6 0 1 0 0-12H8a6 6 0 0 0-5.21 8.98l.01.02a1 1 0 1 0 1.72-1.03z"/>
                                        </svg>
                                        twitter.com/webpraktikos</a>
                                </li>
                                <li class="mt-1 pl-7  text-gray-700 leading-normal">
                                    <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 24 24">
                                        <path class="text-gray-600 print:text-gray-900 fill-current"
                                              d="M5.64 16.36a9 9 0 1 1 12.72 0l-5.65 5.66a1 1 0 0 1-1.42 0l-5.65-5.66zm11.31-1.41a7 7 0 1 0-9.9 0L12 19.9l4.95-4.95zM12 14a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm0-2a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/>
                                    </svg>
                                    1020 Wien, Austria
                                </li>
                                <li class="mt-1 pl-7  text-gray-700 leading-normal">
                                    <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 24 24">
                                        <path class="text-gray-600 print:text-gray-900 fill-current"
                                              d="M4 4h16a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6c0-1.1.9-2 2-2zm16 3.38V6H4v1.38l8 4 8-4zm0 2.24l-7.55 3.77a1 1 0 0 1-.9 0L4 9.62V18h16V9.62z"/>
                                    </svg>
                                    hi@webpraktikos.com
                                </li>
                                <li class="mt-1 pl-7  text-gray-700 leading-normal">
                                    <svg class="w-5 h-5 inline -ml-6 mr-1 -mt-1" xmlns="http://www.w3.org/2000/svg"
                                         viewBox="0 0 24 24">
                                        <path class="text-gray-600 print:text-gray-900 fill-current"
                                              d="M13.04 14.69l1.07-2.14a1 1 0 0 1 1.2-.5l6 2A1 1 0 0 1 22 15v5a2 2 0 0 1-2 2h-2A16 16 0 0 1 2 6V4c0-1.1.9-2 2-2h5a1 1 0 0 1 .95.68l2 6a1 1 0 0 1-.5 1.21L9.3 10.96a10.05 10.05 0 0 0 3.73 3.73zM8.28 4H4v2a14 14 0 0 0 14 14h2v-4.58l-4.5-1.5-1.12 2.26a1 1 0 0 1-1.3.46 12.04 12.04 0 0 1-6.02-6.01 1 1 0 0 1 .46-1.3l2.26-1.14L8.28 4z"/>
                                    </svg>
                                    (+43) 0699 77 33 77 55
                                </li>
                            </ul>
                        </section>

                    </div>

                </section>

        </div>


    </div>
    <!-- end Page -->
</div>

<script src="https://cdn.jsdelivr.net/npm/vue@2.6.0"></script>
<script type="text/javascript">

    var app = new Vue({
        el: '#cv',
        data: {
            message: 'Hello Vue!',
            skills: [
                {
                    title: 'PHP - Laravel',
                    sub_title: 'Middle Level',
                    description: 'Familiarity with OOP design principles Good parts: pure function, higher-order functions, factory functions, composition. Bad parts: inheritance, this, new.',
                    tags: [
                        {item: 'OOP'}, {item: 'MVC'}, {item: 'API'}, {item: 'Laravel'}, {item: 'TDD'}]
                },
                {
                    title: 'JavaScript',
                    sub_title: 'Middle Level',
                    description: 'Good parts: pure function, higher-order functions, factory functions, composition. Bad parts: inheritance, this, new.',
                    tags: ['ES6', 'Vue.js', 'Webpack', 'jQuery']
                },
                {
                    title: 'MySQL',
                    sub_title: 'Middle Level',
                    description: 'Good parts: pure function, higher-order functions, factory functions, composition. Bad parts: inheritance, this, new.',
                    tags: ['ES6', 'Vue.js', 'NPM', 'Webpack', 'jQuery']
                },
                {
                    title: 'CSS3/HTML5',
                    sub_title: 'Middle Level',
                    description: 'Good parts: pure function, higher-order functions, factory functions, composition. Bad parts: inheritance, this, new.',
                    tags: ['Tailwind CSS', 'Bootstrap', 'jQuery']
                },
                {
                    title: 'Others',
                    sub_title: 'Middle Level',
                    description: 'Good parts: pure function, higher-order functions, factory functions, composition. Bad parts: inheritance, this, new.',
                    tags: ['GIT', 'Unix Shell', 'MacOS', 'Linux', 'Visual Studio Code', 'PHPStorm',]
                },

            ]
        },
        mounted() {
            console.log('Done!')
        }
    })
</script>

</body>
</html>
